<template>
  <layout title="Pagination" source="pagination/index.vue">
    <div class="components-view">
      <div class="view-title">基础实例</div>

      <mod-pagination v-model="current"  :pageCount="5" v-on:active="onActive"></mod-pagination>
    </div>

    <div class="components-view">
      <div class="view-title">外部控制页数更改</div>

      <mod-button :inline="true" v-on:click="current = 2">点击跳转到第三页</mod-button>
    </div>

    <div class="components-view">
      <div class="view-title">页数特殊情况 1，2，3 页</div>

      <mod-pagination :pageCount="1" v-on:active="onActive"></mod-pagination>
      <mod-pagination :pageCount="2" v-on:active="onActive"></mod-pagination>
      <mod-pagination :pageCount="3" v-on:active="onActive"></mod-pagination>
    </div>


    <div class="components-view">
      <div class="view-title">使用作用域插槽实现自定义样式</div>

      <mod-pagination v-model="current"  :pageCount="5" v-on:active="onActive">
        <template slot="item" scope="props" >
          <a v-if="props.type == PAGE_TYPE.PAGE || props.type == PAGE_TYPE.CURRENT"
            href="javasciprt:void(0)" :class="{'custom-link': true, 'custom-link-active': props.type == PAGE_TYPE.CURRENT}">{{ props.page + 1 }}</a>

          <a href="javasciprt:void(0)" class="custom-link" v-if="props.type == PAGE_TYPE.FIRST">首</a>
          <a href="javasciprt:void(0)" class="custom-link" v-if="props.type == PAGE_TYPE.LAST">尾</a>


          <a href="javasciprt:void(0)" class="custom-link" v-if="props.type == PAGE_TYPE.PRE">上</a>
          <a href="javasciprt:void(0)" class="custom-link" v-if="props.type == PAGE_TYPE.NEXT">下</a>

          <a v-if="props.type == PAGE_TYPE.OMIT">...</a>

        </template>
      </mod-pagination>

    </div>
  </layout>
</template>

<script>
  import Layout from '../common/common_layout';

  export default {
    data () {
      return {
        current: 0,
        PAGE_TYPE: this.$jsmod.pagination.PAGE_TYPE
      }
    },
    methods: {
      onActive (e) {
        this.$jsmod.toast.show({
          content: '启动了：' + (e.page + 1) + '页'
        });
      }
    },
    components: {
      Layout
    }
  }
</script>

<style lang="stylus">
  @import "~website/assets/mixin"

  .custom-link
    border-radius: 15px;
    font-size: 12px;
    background: #efefef;
    color: #333;
    display: inline-block;
    width: 30px;
    line-height: 30px;
    text-align: center;

    &.custom-link-active
      background: main-color;
      color: #fff;

</style>
