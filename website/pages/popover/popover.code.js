...
code.overview.en = `
\`ModPopover\` component is the basis for all popover components, \`tooltip\`, \`dropdown\` was extended by it. component provide rich api, also can remove default style. developers can customize the style.

* don't need to define the width and height, it can automatically calculate(update) position
* arrow's size, color, border-color can be configured
...
code.position.en = `
The targetType parameter consists of three fields, each of which has an optional value of \`top \`, \`bottom \`, \`left \`, \`right \`, \`center \` (center can be omitted)

For the parameter \`top, left, right \` explained as follows：

1. the first value \`top\`, indicates above the target element
2. the second value \`left\` indicates in the middle of the upper left corner of the target element
3. the third value \`right\` indicates in the right of the upper left corner of the target element
`
